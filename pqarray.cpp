/*
 * This file takes in from the pqarray.h file
 * and implements the funtionailty
 */
#include "pqarray.h"
#include "error.h"
#include "random.h"
...
/*
 * This function enqueues an element and swaps until the
 * lowest priorty number is at the end.
 */
void PQArray::enqueue(DataPoint elem) {
    ...
    // index > 0 safegaurd
    while (index > 0 && elem.priority > _elements[index - 1].priority) {
...
STUDENT_TEST("Confirm the contents of the internal array before/after enlarging") {
    PQArray pq;

    Vector<DataPoint> contents;

    for (int i = INITIAL_CAPACITY; i >= 1; i--) {
        contents.add({"Person" + integerToString(i), i + 0.0});
    }
    cout << contents << endl;

    pq.debugSetInternalArrayContents(contents, INITIAL_CAPACITY);

    EXPECT_EQUAL(pq.size(), INITIAL_CAPACITY);
    EXPECT_EQUAL(pq.debugGetInternalArrayContents(), contents);

    pq.enqueue({"Person11", 11});

    Vector<DataPoint> contentsPlusOne;

    for (int i = INITIAL_CAPACITY + 1; i >= 1; i--) {
        contentsPlusOne.add({"Person" + integerToString(i), i + 0.0});
    }
    cout << contentsPlusOne << endl;

    EXPECT_EQUAL(pq.size(), INITIAL_CAPACITY + 1);
    EXPECT_EQUAL(pq.debugGetInternalArrayContents(), contentsPlusOne);

}

STUDENT_TEST("Client Side/multiple enqueueueueueueueueueueues") {
    PQArray pq;

    for(int i = 0; i < INITIAL_CAPACITY * 5; i ++) {
        pq.enqueue({"Person" + integerToString(i), i + 0.0});
    }


    EXPECT_EQUAL(pq.size(), INITIAL_CAPACITY * 5);
}
...
STUDENT_TEST("PQArray: time enqueue") {
    PQArray pq;
    Vector<int> inputs = {40000, 60000, 80000, 100000, 120000};
...
STUDENT_TEST("PQArray: time dequeue") {
    PQArray pq;
    Vector<int> inputs = {40000, 60000, 80000, 100000, 120000};
